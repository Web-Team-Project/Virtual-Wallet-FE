...
const fetchCategoriesServer = async () => {
  ...
  const data = await createGetFetch(
...
const deleteCategoryServer = async (name: string) => {
  ...
  const response = await fetch(
    `http://localhost:8000/api/v1/categories?category_name=${encodeURIComponent(
      name
    )}`,
    {
      method: "DELETE",
...
const handleViewProfile = async () => {
  ...
  if (user === undefined) {
    ...
    const verified = await jwtVerify(
      user.value,
      new TextEncoder().encode("yoursecretkey")
    );
    const response = await fetch(
      `http://localhost:8000/api/v1/users/${verified.payload.email}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
const createTransactionServer = async (transaction: {
  amount: number;
  category: string;
  card_number: string;
...
const updateUserRoleServer = async (userId: string, newRole: string) => {
  ...
  const data = await createPutFetch(
    `http://localhost:8000/api/v1/users/${userId}/role`,
    user,
    { role: newRole }
  );
  return data;
...
const deactivateUserServer = async (userId: string) => {
  ...
  const data = await createDeleteFetch(
    `http://localhost:8000/api/v1/users/${userId}/deactivate`,
    user
  );
  return data;
};
...
const blockUserServer = async (userId: string) => {
  ...
  const data = await createPutFetch(
    `http://localhost:8000/api/v1/users/${userId}/block`,
    user,
    {}
  );
  return data;
...
const unblockUserServer = async (userId: string) => {
  ...
  const data = await createPutFetch(
    `http://localhost:8000/api/v1/users/${userId}/unblock`,
    user,
    {}
  );
  return data;
...
const fetchUsersServer = async (
  ...
  const url = `http://localhost:8000/api/v1/search/users?search=${encodeURIComponent(
    search
