...
interface TransactionListProps {
  ...
  onAction: (
    id: string,
    action: "approve" | "reject" | "confirm" | "deny"
  ) => void;
  isAdmin: boolean;
}
            ...
            <table className="w-full border-collapse text-white bg-gray-700 rounded-lg shadow-lg overflow-hidden">
              <thead className="bg-gray-800">
                <tr>
                  ...
                  <th className="text-center border px-2 py-1">
                    Category Name
                  </th>
                  <th className="text-center border px-2 py-1">Card Number</th>
                  <th className="text-center border px-2 py-1 max-w-[150px] truncate">
                    Recipient Email
                  </th>
                  <th className="text-center border px-2 py-1 max-w-[100px] truncate">
                    Status
                  </th>
              ...
              <tbody>
                {transactions.map((transaction) => (
                  ...
                  >
                    ...
                    <td className="text-center border px-2 py-1 max-w-[150px] truncate">
                      {transaction.recipient_email}
                    </td>
                    <td className="text-center border px-2 py-1 max-w-[100px] truncate">
                      {transaction.status}
                    </td>
                    <td className="text-center border px-4 py-4">
                      {transaction.timestamp.toString().slice(0, 10)}
                    </td>
                    <td className="text-center border px-2 py-1">
